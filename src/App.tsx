...
import { Github, Share2, Home, ChevronLeft, LayoutGrid } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <div className="fixed top-0 left-0 w-full z-50 pointer-events-none flex justify-center">
        <motion.header 
          ...
          className={cn(
            "h-14 md:h-16 flex items-center justify-between sticky top-0 z-40 backdrop-blur-md pointer-events-auto transition-all duration-500 border-slate-200/60",
        ...
        >
          <div className="flex items-center gap-2 md:gap-4">
            ...
            >
              <div className="w-8 h-8 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-200 group-hover:scale-110 transition-transform shrink-0">
                <LayoutGrid size={16} />
              </div>
              <span className={cn(
            ...
            {view !== 'home' && (
              <>
                <div className="h-4 w-px bg-slate-200 mx-1 md:mx-2" />
                <button 
                  onClick={view === 'note' ? handleBackToCategory : handleGoHome}
                  className="flex items-center gap-1 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors whitespace-nowrap"
                >
                  <ChevronLeft size={16} />
                  <span className={cn(
          ...
          <div className="flex items-center gap-1 md:gap-3">
            ...
            <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-500 transition-colors hidden sm:flex">
              <Share2 size={18} />
            </button>
            <a 
      ...
      {/* Footer */}
      <footer className="py-12 border-t border-slate-100 bg-slate-50/50 mt-20">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-slate-200 rounded-lg" />
            <span className="text-sm font-bold text-slate-400">Obsidian Garden</span>
          </div>
          <div className="flex gap-8 text-sm text-slate-400">
            <button onClick={handleGoHome} className="hover:text-indigo-600 transition-colors">首页</button>
            <a href="#" className="hover:text-indigo-600 transition-colors">关于</a>
            <a href="#" className="hover:text-indigo-600 transition-colors">归档</a>
          </div>
