      ...
      <div className="max-w-7xl mx-auto flex justify-center px-6">
        <div className="flex-1 max-w-3xl py-12 md:py-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <header className="mb-12">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-2 text-indigo-600 text-xs font-bold uppercase tracking-widest">
                <span className="px-3 py-1 bg-indigo-50 rounded-full">{note.category}</span>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-slate-500 border-y border-slate-100 py-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-2">
                  {note.tags.map(tag => (
                    <span key={tag} className="hover:text-indigo-600 cursor-pointer transition-colors">#{tag}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <article className="prose prose-slate prose-indigo max-w-none 
            ...
            prose-blockquote:border-l-4 prose-blockquote:border-indigo-500 prose-blockquote:bg-indigo-50/30 prose-blockquote:py-2 prose-blockquote:px-6 prose-blockquote:rounded-r-2xl prose-blockquote:not-italic prose-blockquote:text-slate-700
            prose-code:text-indigo-600 prose-code:bg-indigo-50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-md prose-code:before:content-none prose-code:after:content-none prose-code:font-medium
            prose-pre:bg-slate-900 prose-pre:shadow-2xl prose-pre:rounded-2xl prose-pre:p-6
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {relatedNotes.map(n => (
                  <button
                    ...
                    className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-indigo-100 hover:shadow-xl hover:shadow-indigo-500/5 transition-all text-left group"
                  >
                    <div className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-2">{n.category}</div>
                    <h4 className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors mb-2">{n.title}</h4>
          ...
          <footer className="mt-20 pt-12 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-6">
            <button 
            ...
            <div className="text-slate-400 text-xs font-mono bg-slate-50 px-3 py-1.5 rounded-full">
