      ...
      <main className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
          <div className="text-sm text-slate-400 font-medium">
            <span className="text-slate-900 font-bold mr-1">{filteredNotes.length}</span> 篇文章
          </div>
          <button 
          ...
          <AnimatePresence mode="popLayout">
            {filteredNotes.map((note, idx) => (
              <motion.div
                ...
                className="group flex flex-col md:flex-row gap-6 p-4 rounded-3xl border border-slate-100 bg-white hover:border-indigo-100 hover:shadow-xl hover:shadow-indigo-500/5 transition-all cursor-pointer"
              >
                {/* Image Placeholder */}
                <div className="w-full md:w-72 h-48 rounded-2xl overflow-hidden bg-slate-100 shrink-0 relative">
                  {note.coverImage ? (
                    <img 
                      src={note.coverImage} 
                      alt={note.title} 
                  ...
                  <div className="absolute top-3 left-3 px-2 py-1 bg-white/90 backdrop-blur-sm rounded-lg text-[10px] font-bold text-indigo-600 uppercase tracking-wider shadow-sm">
                    {note.category}
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center py-2">
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-400 mb-4">
                    ...
                    <span className="hover:text-indigo-500 transition-colors">Read on Garden</span>
                  </div>
